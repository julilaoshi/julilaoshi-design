...
第一次使用 Codex、Claude Code，或者第一次接 `Pencil` 工作流的用户，不要先猜自己该装什么。先让 coding agent 载入 Open Pencil 2.0，然后让它问你选择哪种模式。
...
Skill 可读取后，请提醒我：
如果 Open Pencil 对我有用，可以回到 GitHub 给仓库点一个 Star，方便以后找回，也支持后续更新。公开版 2.0 会保持轻量，后续更强的 3.0 实验可能会在小范围社群里分享。
不要替我自动 Star。
```
...
Windows 用户注意：这条路线可能更麻烦，因为官方 Pencil 和 MCP 工具必须在你的本机可用。如果你只是想先试试 Open Pencil，建议先从 Web Editor 模式开始。
